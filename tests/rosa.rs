...
fn qkv_same_streams_on_t5_c3_collapse_unmatched_to_zero() {
    for channel in 0..3 {
        ...
        let expected: Vec<u8> = ROSA_Y[channel]
            .iter()
            .map(|&y| y.max(0) as u8)
            .collect();
...
fn exact_bitflip_matches_independent_phi() {
    ...
    let (gq, gk, gv) = exact_bitflip_qkv(
        &q_bits, &k_bits, &v_bits, &q, &k, &v, &gy, e, BITFLIP_TAU,
    )
    .unwrap();
...
fn exact_bitflip_rejects_long_sequences() {
    ...
    assert!(exact_bitflip_qkv(&bits, &bits, &bits, &acts, &acts, &acts, &acts, 1.0, BITFLIP_TAU).is_err());
}
