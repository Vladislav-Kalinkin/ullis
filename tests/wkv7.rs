use ullis::wkv7::{
    CHUNK_LEN, HEAD_SIZE, fixture_t16_h1, wkv7_backward, wkv7_forward, wkv7_step,
};
...
fn fixture_t16_h1_is_checked_in() {
    ...
    assert!(fwd.y.iter().chain(&fwd.s).chain(&fwd.sa).all(|x| x.is_finite()));
...
fn metal_matches_cpu_on_fixture() {
    ...
    let gpu = runtime
        ...
        .unwrap();
    assert!(max_abs(&cpu.y, &gpu.y) < 2e-4, "y {}", max_abs(&cpu.y, &gpu.y));
    assert!(max_abs(&cpu.s, &gpu.s) < 2e-4, "s {}", max_abs(&cpu.s, &gpu.s));
